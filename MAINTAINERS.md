...
模板由 `python-docx + OOXML` 从代码生成。修改字体、编号、快捷键、通用样式、分节、页码、页眉或占位内容时，只需要修改生成逻辑，再统一生成全部成品。
...
书籍版使用三个 Word 节：独立书名页；前言与目录前置部分；正文与后置部分。前置部分页码为小写罗马数字并从 `i` 开始，正文换节后以阿拉伯数字从 `1` 重启。正文页眉使用 `STYLEREF "Title"`，页脚使用 `PAGE` 字段。
...
这些都属于**结构与布局自动检查**，不会启动 Microsoft Word。不要在文档或 Release 说明中把它描述成“所有版本 Word 真机测试通过”。真实 Word 版本、输入法、插件和系统快捷键冲突仍应通过实际使用反馈补充验证。

## 真实 Word 反馈闭环

v2.5.0 起，真实使用反馈分成两条路径：
...
v2.6 起，完整验证表单另外提供“页码 / 页眉”可选测试项，用于检查书籍结构版的罗马页码、正文重启和书名页眉，以及文章结构版的页码。
