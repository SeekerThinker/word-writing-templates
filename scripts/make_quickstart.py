...
def make(platform):
    ...
    txt(c,x,y,f'{label}｜先让思路有结构，再决定是否继续展开',13); y-=10*mm

    txt(c,x,y,'第一次使用',13); y-=8*mm
    steps=[
      '1. 选择更接近你的结构：书籍 / 长篇，或文章 / 日常整理。',
      '2. 双击 .dotx。Word 会创建新文档，原模板不会被改坏。',
      '3. 大主题用“标题 1”；需要拆分时用“标题 2 / 3 / 4”。',
      '4. 积累一些内容后打开“导航窗格”，只看标题检查整体结构。',
    ]
    for s in steps: txt(c,x,y,s,10.5); y-=7.2*mm

    y-=2*mm
    txt(c,x,y,'结构五问：并列吗？单一吗？完整吗？重复吗？顺序对吗？',10); y-=8*mm
    txt(c,x,y,'可以只用来整理思路和积累笔记；需要成稿时，再沿用已经形成的结构。',9.5); y-=6.5*mm
    txt(c,x,y,'前言、摘要、目录、参考文献等可选区块不会进入导航窗格。',9.5); y-=6.5*mm
    txt(c,x,y,'导航窗格主要显示你的标题层级，也就是思考结构。',9.5); y-=10*mm
