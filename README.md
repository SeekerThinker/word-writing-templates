...
书籍结构版包含独立书名页、作者占位、前言、自动目录、正文、附录和参考文献。前言与目录使用小写罗马页码 `i, ii, iii…`，正文换节后从阿拉伯数字 `1` 重新开始。

**v2.7 的书籍结构版进一步面向长文打印：**每个“标题 1”自动另起新页；启用镜像页边距并预留装订线；正文奇数页显示当前章节页眉、偶数页显示书名；页码位于页面外侧。模板不会强制每章从奇数页开始，因此不会为了排右页而在写作阶段自动塞入空白页。

文章结构版包含作者、单位、日期、摘要、关键词、正文、参考文献以及从第 `1` 页开始的页码；v2.7 同时微调了标题与作者信息之间的视觉节奏。
...
后面需要新章节时，使用 Word 的“标题 1 / 标题 2 / 标题 3 / 标题 4”样式即可。快捷键只是更快，不是必需条件。书籍结构版中，“标题 1”还会自动从新页开始。
...
项目会对 **24 个成品模板**（12 个直接开始版 + 12 个带常用结构版）做自动结构检查，并在 GitHub Actions 的 **Windows、macOS 和 Linux** 环境中重复验证：模板包可读取、四级编号存在、平台字体设置正确、快捷键映射存在、中文路径可读取，并且不包含 VBA 宏。

成稿结构检查还会验证书籍分节、罗马 / 阿拉伯页码切换、页码重启、镜像页边距、装订线、章节自动换页、奇偶页页眉 / 页脚、`STYLEREF` / `PAGE` 字段，以及文章元信息结构。构建流程会用 LibreOffice 把全部 24 个模板转换为 PDF 做布局冒烟检查。
